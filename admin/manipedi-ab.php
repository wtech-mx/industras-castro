...
if(isset($_GET['delete_id'])){
	...
	$stmt_select = $DB_con->prepare('SELECT Imagen_Img FROM manipedi WHERE Imagen_ID =:uid');
	...
	$stmt_delete = $DB_con->prepare('DELETE FROM manipedi WHERE Imagen_ID =:uid');
	$stmt_delete->bindParam(':uid',$_GET['delete_id']);
	$stmt_delete->execute();
	// Redireccioa al inicio
	header("Location: manipedi-ab.php");
}
...
<div class="contenedor p-5">
	  ...
	  <div class="col-xs-12 text-right">
		  <a href="Agregarmanipedi.php" class="btn btn-default" ><span class="glyphicon glyphicon-plus"></span> Agregar </a>
	  </div>
	...
	$stmt = $DB_con->prepare('SELECT Imagen_ID, Imagen_Marca, Imagen_Tipo, Imagen_Img, Imagen_Img2 FROM manipedi ORDER BY Imagen_ID DESC');
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<div class="col-md-12" style="padding: 30px;">
				...
				<td>
					<a class="btn btn-warning btn-sm" href="Editarmani-bf.php?edit_id=<?php echo $row['Imagen_ID']; ?>">Editar</a>
