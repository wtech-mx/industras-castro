...
{
	$id = $_GET['edit_id'];
	$stmt_edit = $DB_con->prepare('SELECT Imagen_Marca, Imagen_Tipo, Imagen_Img, Imagen_Img2 FROM faciales WHERE Imagen_ID =:uid');
...
if(isset($_POST['btn_save_updates']))
{
	...
	if(!isset($errMSG)){
		$stmt = $DB_con->prepare('UPDATE faciales SET Imagen_Marca=:uname, Imagen_Tipo=:ujob, Imagen_Img=:upic, Imagen_Img2=:upic2 WHERE Imagen_ID=:uid');
...
<script>
			...
			window.location.href='faciales-ab.php';
			</script>
<?php
		}
		else{
